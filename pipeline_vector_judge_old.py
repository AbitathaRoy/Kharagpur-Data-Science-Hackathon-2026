...
def evaluate_consistency(claim, constraints):
    ...
    if not flat_constraints:
        return "consistent", "No evidence found."
    ...
    # --- THE SKEPTICAL JUDGE ---
    prompt = f"""
    You are a Continuity Editor. Check if the User Claim contradicts the Novel Evidence.
    ...
    INSTRUCTIONS:
    1. **Search for Conflicts:** Does the claim contradict dates, names, locations, or *character personality* described in the evidence?
    2. **Silence is Okay:** If the claim adds new details that *fit* the character/world, verdict "consistent".
    3. **Contradiction is Fatal:** If the claim violates an established fact (e.g., Claim says "Alive in 1820" but Evidence says "Died 1815"), verdict "contradict".
    
    CRITICAL: 
    - If the evidence describes a specific event (e.g., "Father died in 1815"), and the claim describes it differently (e.g., "Father died in 1820"), that is a CONTRADICTION.
    - If the evidence describes a character as "Peaceful", and the claim says they "Massacred a village", that is a CONTRADICTION.
    
    Return JSON: {{ "verdict": "consistent" | "contradict", "reason": "Explanation" }}
    """
...
def run_vector_pipeline():
    ...
    for i, item in enumerate(claims_data):
        ...
        for p in probes:
            # In run_vector_pipeline() loop:
            # Change top_k from 5 to 15
            hits = engine.search(p, top_k=15)
