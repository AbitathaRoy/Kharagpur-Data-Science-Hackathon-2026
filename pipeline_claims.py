...
"""
Ingest claims/content from train.csv and convert to json.
Now optimized for Caching in extraction/caption_claims.py
"""
...
DELAY_SECONDS = 0.5  # Kept small to be polite, but not strictly needed for Cache Hits
    ...
    try:
        with open(file_path, mode='r', encoding='utf-8-sig') as f:
            ...
            for i, row in enumerate(reader):
                ...
                # Also support 'input_text' if csv header varies
                if not text_content:
                    text_content = row.get("input_text", "").strip()
                ...
                items.append({
                    ...
                    "metadata": {
                        ...
                        "character": row.get("char", row.get("character")), # Handle 'char' vs 'character'
...
def main():
    print("--- Starting Extraction Pipeline (Cached) ---")
    ...
    for i, item in enumerate(data_items):
        ...
        try:
            # --- THE CORE CALL (Handles Caching Internally) ---
            ...
            if is_dataclass(claims_obj):
                ...
                try:
                    ...
                    try:
                        ...
                        claims_dict = claims_obj.__dict__ # Plain class
        ...
        # Sleep only slightly to avoid spamming terminal or hitting rare race conditions
        if DELAY_SECONDS > 0:
